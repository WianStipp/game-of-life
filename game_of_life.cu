...
__device__ int count_neighbors(int a[GRID_SIZE][GRID_SIZE], int x, int y)
{
    int i, j;
    int sum = 0;
    for (i = -1; i < 2; i++)
...
__global__ void update_next_state(int a[][GRID_SIZE], int next_state[][GRID_SIZE])
{
    int x = threadIdx.x;
    int y = threadIdx.y;
    int num_neighbors;
...
__global__ void device_set_state_as_next(int a[][GRID_SIZE], int b[][GRID_SIZE])
{
    int x = threadIdx.x;
    int y = threadIdx.y;
    a[x][y] = b[x][y];
